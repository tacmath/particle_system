...
void VAO::LinkAttrib(VBO& vbo, GLuint layout, GLuint numComponentsPerVertex, GLuint type, GLuint typeSize, GLuint offset) {

    glEnableVertexArrayAttrib(ID, layout);
    glVertexArrayAttribBinding(ID, layout, 0);
    glVertexArrayAttribFormat(ID, layout, numComponentsPerVertex, type, GL_FALSE, offset);

    glVertexArrayVertexBuffer(ID, 0, vbo.ID, 0, numComponentsPerVertex * typeSize);
}
